...
test("server-renders the Qwen-UI-Agent technical report", async () => {
  ...
  assert.match(html, /@article\{zhou2026qwen_ui_agent/);
  assert.match(html, /arXiv preprint arXiv:2607\.28227/);
  assert.match(html, /BIBTEX · CITATION/);
  ...
  const weeklyNewspaperIssue22 = await readFile(
    new URL(
      "../public/weekly-newspaper/issue-22/index.html",
      import.meta.url,
    ),
    "utf8",
  );
  assert.match(weeklyNewspaperIssue22, /AI Newspaper · Issue 22/);
  assert.match(
    weeklyNewspaperIssue22,
